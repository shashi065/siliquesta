// Enhanced API client with robust error handling and validation.
...
async function awaitJobResult<T>(
  jobId: string,
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    try {
      ...
      if (pollCount % 5 === 0) {
        console.warn(`Poll ${pollCount} failed for job ${jobId}, retrying...`, err);
      }
    }
  ...
  throw new APIError(504, `Job ${jobId} timed out after ${(Date.now() - started) / 1000}s`, {
    pollCount,
    jobId,
...
async function submitJob<T>(
  path: string,
  payload?: unknown,
  options?: { timeout?: number; validateResult?: boolean }
): Promise<T> {
  ...
  if (job.status === 'queued' && job.job_id) {
    try {
      const result = await awaitJobResult<T>(
        job.job_id,
      ...
      if (options?.validateResult) {
        if (path.includes('simulate')) {
          if (!validateSimulationResult(result)) {
            throw new APIError(422, 'Simulation result validation failed', result);
          }
        } else if (path.includes('optimize')) {
          if (!validateOptimizationResult(result)) {
            throw new APIError(422, 'Optimization result validation failed', result);
          }
        }
      }
      ...
      throw new APIError(500, 'Job execution failed', { jobId: job.job_id, error: err });
    }
  }
  
  // If not queued, return result directly (synchronous endpoint)
...
export const simulationAPI = {
  run: (params: {
    ...
    corner?: string;
    tech_node?: number;
  }) => submitJob('/simulate', params, { validateResult: true }),
  ...
  sweep: (params: {
    ...
    submitJob(`/simulate/sweep?max_wn=${params.wn_max}`, {
      ...
      temp: params.temp,
      cl_ff: params.cl_ff,
      corner: params.corner || 'tt',
      tech_node: params.tech_node || 28,
    }),
...
export const optimizerAPI = {
  run: (params: {
    ...
    min_freq?: number;
    objective?: string;
  }) => submitJob('/optimize', params, { validateResult: true }),
  ...
  mlOptimize: (params: {
    ...
    tech_node?: number;
    objective?: string;
  }) => submitJob('/optimize/ml-optimize', params, { validateResult: true }),
  ...
  predict: (params: {
    ...
    cl_ff?: number;
    tech_node?: number;
  }) => submitJob('/optimize/predict', params),
};
